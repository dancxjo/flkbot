...
module.exports = class Chatbot {
    ...
    tryToTranslateEnglish() {
        this.youCanSay('Du kan frege mek tu oversete en englisk vord in ander sprak. Hur seg man "popcorn" in dütisk?');
        
        this.listenFor(/(?:Hur|(?:Vilk mot)) seg (?:man|maner|du|di|vi)(?: de englisk(?: vord)?)? "(.+?)" in (.+?)\?/i, (from, to, matches) => {
            const word = matches[1];
            const destination = matches[2];
            console.log(`Attempting to translate English ${word} into ${destination}`);
            this.translate(to, from, word, 'englisk', destination);
        });
    }
